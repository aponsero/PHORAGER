...
- **Containerized run workflows failed on Apptainer with `FileNotFoundError` on every Nextflow-staged input**
  - Nextflow stages process inputs into the work directory as symlinks pointing at the original files (e.g. `.../fasta/GCF_....fna`). The pipeline invokes `singularity exec` manually in process script blocks rather than through Nextflow's container integration, so `singularity.autoMounts` and `singularity.runOptions` never apply — neither profile sets `singularity.enabled`, and Nextflow therefore manages no containers. With no automatic mounting, the symlink *targets* were not bound into the container: the links were visible but dangling, and any tool following them failed. `RENAME_CONTIGS` was the first casualty, with BioPython raising `FileNotFoundError` on a staged genome. Confirmed directly — `ls -la` inside the container showed the symlink and its target, while `cat` on the same file returned "No such file or directory", and binding the resolved parent directory made the identical `cat` succeed
  - This affected every run-workflow process that execs a container over staged inputs, not just `RENAME_CONTIGS`; the existing ad-hoc `--bind $(pwd)` calls in some processes only covered inputs resolving inside the work tree, so coverage was inconsistent across modules
  - Fixed globally rather than per-process: `nextflow.config` now computes a bind string and exports it as `APPTAINER_BIND`/`SINGULARITY_BIND` via the `env {}` scope. Apptainer and Singularity read these from the environment and apply them to *manual* `singularity exec` calls, additively with any per-call `--bind`, so no process module required editing and existing binds continue to work. Both variable names are set: Apptainer reads `APPTAINER_BIND`, older SingularityCE reads `SINGULARITY_BIND`
  - The bind list covers every root a staged symlink can resolve into: `params.outdir` (which contains the work directory, since `workDir = "${params.outdir}/tmp"`), `params.database_location`, and the resolved parent of `params.genome` / `params.prophage` (the directory itself if the input is a directory). A new `params.extra_bind_paths` (comma-separated, default `null`) lets users add binds for unusual site layouts without editing pipeline code
  - Two follow-on defects fixed during implementation: (a) under Nextflow 26.04's strict config parser, top-level `def`/imperative statements cannot be mixed with config blocks, so the bind logic is expressed as a closure assigned to `params.phorager_bind_str` and invoked with `.call()`; (b) the bind list initially passed `params.outdir` and `params.database_location` through verbatim, so a relative `--outdir test_xaa` produced `FATAL: unable to add test_xaa to mount list: destination must be an absolute path` (exit 255) — every element is now canonicalized to an absolute path
  - Verified: `nextflow config -profile singularity` resolves `APPTAINER_BIND`/`SINGULARITY_BIND` to absolute paths only; the prophage and annotation workflows now run to completion on Apptainer 1.5.2

- **Relative `--genome`, `--prophage`, and `--outdir` paths were silently resolved against the pipeline directory instead of the user's working directory**
  - `phorager prophage --genome fasta` failed with `Input path does not exist or is not accessible: fasta` even with `fasta/` present in the user's current directory. The Python wrapper's command classes call `os.chdir(phorager_dir)` before invoking Nextflow, but passed `--genome`/`--prophage`/`--outdir` through as the raw user-typed strings, while `--database_location` and `--singularity_cache_dir` were already absolute from config. A relative input was therefore resolved by Nextflow against `PHORAGER/` rather than the user's shell directory. Input validation runs *before* the `chdir`, so it passed and the failure surfaced later inside the workflow, obscuring the cause
  - The same `chdir` explains why default `$projectDir`-anchored `outdir`/`database_location` values resolved under `PHORAGER/`, and it fed relative values into the container bind string
  - Fixed in `prophage.py`, `bacterial.py`, and `annotation.py`: `--genome`/`--prophage` and `--outdir` are now resolved with `Path(...).resolve()` inside `_build_nextflow_command()`, which runs before the `os.chdir()` and therefore resolves against the user's shell directory. Relative and absolute invocations now behave identically. `--database_location` is unchanged (already absolute from config)
  - Verified: `--genome fasta` (relative) and the equivalent absolute path both resolve correctly across all three entry points

- **`CLUSTER_PHAGES` failed at BLAST database creation with a spurious permissions error on NFS**
  - `makeblastdb -out blast_db/filtered_phage_db` aborted with `CBuildDatabase::CreateDirectories() - You do not have write permissions on 'blast_db'`, despite the directory being writable — `touch blast_db/<file>` inside the same container, as the same user, succeeds
  - Isolated by testing the variables independently: `-out testdb_nosubdir` (no directory component) succeeds on the same NFS path; `-out blast_db/testdb` succeeds on node-local `/tmp`. Only the combination of a directory component in `-out` *and* an NFS destination fails. Absolute paths and an explicit `--bind` of the resolved directory made no difference — the error is identical either way, so this is not a path-resolution or mount problem. The fault is BLAST's own writability probe in `CreateDirectories()`, which only executes when `-out` contains a directory component and misreports this filesystem
  - Fixed by removing the directory component: `makeblastdb` now runs with `blast_db` as its working directory and a bare `-out filtered_phage_db`. Applied to both the Singularity and Conda branches — the fault is in BLAST, not in the container runtime, so the Conda path runs the same command against the same filesystem. The `blastn` calls are deliberately unchanged: they only *read* `-db blast_db/...` (no `CreateDirectories`) and write `-out phage_blast.tsv` with no directory component. The `path "blast_db/*"` output declaration is unaffected
  - This is a latent environment/BLAST interaction rather than a regression, but it surfaced during the container rebuild: the replacement bundle pins `blast=2.16.0`, and the retired Sylabs image's BLAST version can no longer be inspected to confirm whether it carried the same probe
...
- **`phorager install --databases checkv` downloaded and extracted the CheckV database but never built its DIAMOND index, failing every attempt with the download always re-run from scratch**
  ...
  - **(b)** Once (a) was fixed and DIAMOND actually ran, a second defect surfaced: the build did `cd "${db_path}/genome_db"` and called `diamond makedb --in checkv_reps.faa` with a *relative* input path. Under Nextflow the process executes from a staged symlink (`databases` → the real database directory), so the container's inherited working directory is the *logical* symlinked path, from which the container cannot resolve the relative filename even though the file is physically present at the resolved path. Confirmed directly: inside the work dir, `singularity exec … pwd` reports the logical symlinked path and `ls checkv_reps.faa` fails there, while the same file exists at the real `.../databases/checkv_database/genome_db/checkv_reps.faa`. Running the identical `diamond makedb` by hand from the resolved path builds the 951 MB `checkv_reps.dmnd` without error
  - Fixed (b) by removing the `cd` + relative-name pattern: the build now resolves `genome_db` with `realpath`, binds it explicitly into the container, and passes absolute paths to both `--in` and `--db`, making the step independent of the container's inherited working directory
  - Net effect: no CheckV database was ever successfully built via this pipeline's Singularity path — the download always completed but the DIAMOND build always failed, so any run with `--databases checkv` produced an incomplete `checkv_database/` with no `checkv_reps.dmnd`. Both fixes are scoped to the Singularity branch; the Conda branch is unchanged
  - Deferred (tracked separately, not addressed here): the download branch begins with `rm -rf ${db_path}`, so a failed or killed attempt destroys a good extraction and forces a full re-download. Extraction is not yet atomic. This is now masked because the step succeeds on the first attempt, but remains worth hardening (extract to a temp path, build, then move into place) to protect against a genuinely truncated download
...
- **Global container bind paths for manual `singularity exec`**: because no profile sets `singularity.enabled`, Nextflow's container layer — including `autoMounts` and `runOptions` — is inert; the only mechanism that reaches hand-written `singularity exec` calls is the runtime's own `APPTAINER_BIND`/`SINGULARITY_BIND` environment variables, exported from `nextflow.config`'s `env {}` scope. The bind string is computed by a closure (`params.phorager_bind_str`, invoked with `.call()`) because Nextflow 26.04's strict config parser rejects top-level imperative statements alongside config blocks. All paths are canonicalized to absolute; Apptainer rejects relative bind sources. The existing `singularity.runOptions = '--cleanenv --no-home'` line is now known to be dead config, and the ad-hoc per-process `--bind $(pwd)` calls are redundant but harmless — both are candidates for a post-v1 cleanup
- **Wrapper path resolution must precede `os.chdir()`**: all three command classes build the Nextflow command before changing into the pipeline directory, so `Path(...).resolve()` in `_build_nextflow_command()` captures the user's shell directory. Reordering those calls would silently reintroduce the relative-path bug
