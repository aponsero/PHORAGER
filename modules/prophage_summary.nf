process PROPHAGE_SUMMARY {
    ...
    # Combine all sequences into one file and get statistics
    ...
    with open("Prophage_detection_summary.log", 'w') as f:
        ...
        f.write(f"Total genomes analyzed: {len(summary_files)}\\n")
        f.write(f"Total prophage regions detected: {total_stats['total_regions']}\\n")
        f.write(f"Total sequences extracted: {total_sequences}\\n")
        f.write(f"Total base pairs in prophage sequences: {total_bp:,}\\n\\n")
        
        f.write("Tool Statistics:\\n")
        f.write("-"*20 + "\\n")
        for tool, count in tool_stats.items():
            f.write(f"{tool}: {count} predictions\\n")
            
        # Add information about combined files
        f.write("\\nOutput Files:\\n")
        f.write("-"*20 + "\\n")
